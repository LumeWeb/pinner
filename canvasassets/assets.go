...
// fully-self-contained ESM bundles built by the JS toolchain (dist/*.js) plus
// the generated mcpcanvas AssetSource manifest (appsassets/manifest.json).
//
...
// dist/ and manifest.json); NewRenderer subs into it.
//
//go:embed appsassets
var Assets embed.FS

// ThemeCSS is the compiled Tailwind v4 theme for ui:// MCP Apps, built from
// css/input.css by the `make assets` pipeline (pnpm build:css). It is the
// single source of the app identity: the @theme tokens (dark zinc surface,
// blue accent, status palette) plus the @utility component classes the view
// bodies (authored in go.lumeweb.com/pinner/canvas) and JS bundles reference.
// The output is tree-shaken to exactly the utilities used across apps.
//
...
// NewRenderer passes it to canvas.NewRenderer as the document theme.
//
