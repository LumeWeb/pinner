...
// Pinner content-network services. It is defined by the Service interface and
// is Output-free.
package websites
...
// shared generic pinner.ListOptions.
...
// generic pinner.ListOptions with the websites filter struct, so paging is
// common to every service and only the filter differs.
type ListOptions = pinner.ListOptions[ListFilter]
...
// long-lived service live-reloads a `pinner login` that
