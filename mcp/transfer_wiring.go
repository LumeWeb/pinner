...
//
// This is the injected-function-type boundary of the package: the descriptor
// halves never construct coordinators or services — the composition root
// wires them (pinnertransfer.StreamUpload / StreamDownload over its own
// services, the HTTP upload/download coordinators it serves on its mux) and
// hands them in.
type TransferDeps struct {
	...
	// ORs it with FileDrop != nil). VaultPutFile/VaultGetFile are not part of
	// this package's registered tool set, but the flags exist so an assembly
	// can still report honestly about them.
	UploadFile   bool
	VaultPutFile bool
	DownloadFile bool
	VaultGetFile bool
...
// nor registered without being advertised.
