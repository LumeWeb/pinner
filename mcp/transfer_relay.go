...
// The upload_url relay tool's description half. The coordinator/executor
// halves stay where the other upload descriptors leave them: ieo.OpenFileURL
// (the vendored, SSRF-hardened URL relay primitive in mcpplane/ieo) fetches the
...
// upload_url is a server-fetch URL
...
func RelayURLUploadDescriptor(handler transfer.UploadHandler, allowedHosts []string, maxBytes int64, features mcpforge.FeatureSet) model.ToolDescriptor {
	...
	if !ok {
		panic("mcp: upload_url has no matching description target for its feature set")
	}
