...
// Characterization of the curated tool-name sets: the full surface exposes
// the seven
...
func TestCuratedToolNamesZeroSurfaceIsFull(t *testing.T) {
	...
	require.Equal(t, CuratedToolNames(assembly.Surface{}), CuratedToolNames(assembly.FullSurface))
}
...
func TestCuratedToolNamesVaultSurfaceKeepsVaultEntries(t *testing.T) {
	...
	s := assembly.Surface{Account: true, Vault: true}
...
func TestStampCuratedMarksOnlyCuratedNames(t *testing.T) {
	...
	stampCurated(CuratedToolNames(assembly.HostedSurface), descs)
