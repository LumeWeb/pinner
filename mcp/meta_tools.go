...
// This file carries the progressive-disclosure meta-tools (search_tools,
// describe_tool, and the typed invoke dispatchers invoke_read_tool /
// invoke_write_tool / invoke_destructive_tool) as part of the assembled
// presentation layer, characterizing the reference behavior introduced on the
// hosted composition (adapted from pinner-cli's internal/mcp/sdk_official.go +
// catalog.go).
//
// The discovery index is NOT a hand-built ToolCatalog: it is derived from this
// assembled Server — the compiled catalog surface (dispatched through the
// owning opmesh.Catalog via the caller-supplied CatalogDispatch seam) plus the
// direct-only tools (agent_guide, capabilities, the wired transfer tools),
// which keep their baked-in handlers.
//
// Registration is controlled ONLY by the resolved listing policy
// (ServesMetaTools): progressive always serves the meta-tools; flat serves
// them unless IncludeMetaOnFlat is explicitly false. This package does not
// wire a protocol server: MetaToolDescriptors returns the SDK-neutral
// descriptors and the composition root registers them.
...
// metaToolValues are the five reserved names in a stable order. The reserved
// check treats them as non-registrable surface names.
var metaToolValues = []string{
	MetaToolSearch,
	MetaToolDescribe,
	MetaToolInvokeRead,
	MetaToolInvokeWrite,
...
// MetaToolNames returns the five progressive-discovery meta-tool names in
// their stable registration order. Callers use it to gate/verify surface
// composition and tests use it to pin the wire names.
func MetaToolNames() []string {
	out := make([]string, len(metaToolValues))
	copy(out, metaToolValues)
	return out
}
...
// shared policy value — never a deployment mode. This is the shared predicate
// for the meta registration gate, mirroring pinner-cli's
// ToolCatalog.servesMetaTools (progressive always; flat omitted only on the
// explicit opt-out).
func ServesMetaTools(listing ListingPolicy) bool {
	return listing.Strategy != ListingFlat || listing.ResolveIncludeMetaOnFlat()
}

// ServesMetaTools reports whether this assembly's resolved listing policy
// keeps the meta-tools on tools/list. Composition roots gate the
// MetaToolDescriptors registration on it.
func (s *Server) ServesMetaTools() bool { return ServesMetaTools(s.listing) }

// CatalogDispatch is the composition-root dispatch seam for catalog
// operations: for a compiled op name it returns the handler that executes the
// op through the owning catalog's gates. The hosted composition roots pass
// their credential-threading dispatch; a local root passes its own. Catalog
// op metadata (search/describe/classify) is served from this Server's
// assembled state; only execution crosses the seam.
type CatalogDispatch func(name string) model.ToolHandler
...
// MetaSurface is the discovery index behind the meta-tools: the compiled
// catalog surface (metadata only — dispatch goes through the owning
// opmesh.Catalog via the CatalogDispatch seam) plus the direct-only tools
// (baked-in handlers). It is immutable once built by newMetaSurface.
type MetaSurface struct {
	// catalog entries dispatch through the caller-supplied catalog seam.
	...
	cat    opmesh.Catalog
	// dispatch executes compiled catalog ops on behalf of the composition
	// root (credential threading and gate application stay with the root).
...
// discoverable, exactly like the CLI assembly's catalog indexes both
// surfaces. It fails when the assembled surface already declares one of the
// reserved meta-tool names — such an op would, if registered directly,
// collide with (or shadow) the dispatcher on tools/list.
func newMetaSurface(s *Server, dispatch CatalogDispatch) (*MetaSurface, error) {
	ms := &MetaSurface{
		catalog:  s.Tools,
		direct:   s.Direct,
		cat:      s.catalog,
	...
	for _, name := range metaToolValues {
		if _, ok, _ := ms.lookup(name); ok {
			return nil, fmt.Errorf("mcp: meta surface: assembled surface declares reserved meta-tool name %q", name)
		}
	}
	return ms, nil
}
...
// surface (the agent guide, capabilities, and the wired transfer tools named
// DirectVisible by the assembled policy), which is exactly the bounded set a
// fresh agent gets before any search.
func (s *MetaSurface) onboard() onboardingResult {
	...
	return onboardingResult{
		Tools: tools,
		Total: len(tools),
		Hint:  "These are the primary start-here tools (agent-guide orientation, capabilities, and the wired file-transfer surface). Call agent_guide for the full ordered flows, or search with a keyword (optionally filtered by category) to discover the full assembled tool surface.",
	}
}

// search returns tools matching a non-empty keyword query, carrying the CLI
...
// enforced its safety class; catalog ops go through the caller-supplied
// CatalogDispatch seam, while direct-only tools call their baked-in handlers.
...
// MetaToolDescriptors builds the five progressive-disclosure meta-tool
// descriptors from this assembled Server's surface. Call only when
// ServesMetaTools() is true; the composition root registers the returned
// descriptors on its protocol server as-is (their handlers are baked in),
// having wired CatalogDispatch to its own catalog dispatch seam. The
// catalog-operated dispatchers route every compiled-op invocation through
// that seam, so the Interaction, Visibility, Safety, and credential gates the
// root applies to its direct tools hold for meta invocations too.
func (s *Server) MetaToolDescriptors(dispatch CatalogDispatch) ([]model.ToolDescriptor, error) {
	ms, err := newMetaSurface(s, dispatch)
	if err != nil {
		return nil, err
	}
	out := make([]model.ToolDescriptor, 0, len(metaToolValues))
	searchDesc, err := metaSearchDescriptor(ms)
	if err != nil {
		return nil, err
	}
	out = append(out, *searchDesc)
	describeDesc, err := metaDescribeDescriptor(ms)
	if err != nil {
		return nil, err
	}
	out = append(out, *describeDesc)
	invokeDescs, err := metaInvokeDescriptors(ms)
	if err != nil {
		return nil, err
	}
	out = append(out, invokeDescs...)
	return out, nil
}

// metaSearchDescriptor builds search_tools, the discovery workflow entry
// point. The description documents the search -> describe -> invoke loop so a
// model discovers without any out-of-band knowledge.
func metaSearchDescriptor(ms *MetaSurface) (*model.ToolDescriptor, error) {
	schema := &metaSchema{}
...
// metaDescribeDescriptor builds describe_tool: full input schema for a single
// tool by name, with "did you mean ...?" recovery for unknown names.
func metaDescribeDescriptor(ms *MetaSurface) (*model.ToolDescriptor, error) {
	schema := &metaSchema{}
...
// metaInvokeDescriptors builds the three typed invoke dispatchers. Each
...
func metaInvokeDescriptors(ms *MetaSurface) ([]model.ToolDescriptor, error) {
	specs := []struct {
		name        string
		title       string
		description string
	...
	out := make([]model.ToolDescriptor, 0, len(specs))
...
// meta discovery behaves identically to the self-hosted surface. ---
