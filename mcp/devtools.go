...
// introspection tools that help debug the MCP server and the host connected
// to it. They are added to the assembled direct surface only when Config
// declares DevTools, so the production surface is unaffected. Each tool is
// DirectVisible, so it appears in tools/list when dev tools are on.
//
// The tools are SDK-free: they read the enriched model.RequestCaps (which
// carries the resolved *model.Profile and, under dev tools, the raw wire
// snapshot), never the go-sdk request directly. Their StructuredContent is a
// typed struct so the result shape is stable and self-documenting.
...
// devClientInfoFrom converts a model client-info into the structured form,
// returning nil when there is nothing to report.
func devClientInfoFrom(ci *model.ClientInfo) *devClientInfo {
	if ci == nil {
		return nil
	}
...
func devUserAgentFrom(profile *model.Profile) *devUserAgent {
	if profile == nil {
		return nil
	}
	var values []string
...
func devProfileFromRequest(request model.ToolRequest) *model.Profile {
	if request.Caps != nil && request.Caps.Profile != nil {
		return request.Caps.Profile
	}
	return devStdioProfile()
}

// devStdioProfile returns the fallback profile for requests without caps
// (direct handler tests, transports that do not derive caps): a co-located
// stdio client of unknown host with the stdio transport mechanism features,
// mirroring the transport-generic fallback the rest of the package resolves.
func devStdioProfile() *model.Profile {
	return &model.Profile{
		HostType:   model.HostGeneric,
		Transport:  model.TransportStdio,
		AuthMethod: model.AuthNone,
		Features: model.FeatureSet{
			model.FeatSourcePath: true,
			model.FeatSinkLocal:  true,
			model.FeatSinkDrop:   true,
			model.FeatCoLocated:  true,
		},
	}
}
...
// DevEnabled reports whether the assembled server carries the dev-tools
// introspection surface on Direct. Composition roots read it to decide
// whether to populate the per-request raw wire snapshot
// (RequestCaps.Capabilities / InitializeParams) that dev_host_env exposes.
func (s *Server) DevEnabled() bool { return s.config.DevTools }
...
// enabledModelFeatures returns the sorted list of feature names a model
// profile enables.
func enabledModelFeatures(profile *model.Profile) []string {
	if profile == nil {
		return nil
	}
	var feats []string
