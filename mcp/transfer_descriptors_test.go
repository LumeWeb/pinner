...
// Characterization tests for the transfer-tool descriptor halves: per-profile
// descriptions, the feature-gated input schemas (source.mode enum, sink enum,
// file handoff), and the archive_mode transform.
...
// TestDownloadSinkSchemaEnum: the comma-enum reflector pitfall must never
// publish a partial sink enum; the
...
// TestUploadSourceModeSchemaEnum: the compiled upload_file
...
// description's feature-gated fragments for the same profiles.
func TestUploadFileDescriptionPerProfile(t *testing.T) {
	...
	// segments appear in the startup-effective description.
