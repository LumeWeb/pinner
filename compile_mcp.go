...
// is the frontend-specific element type: the MCP compiler is a
// Compiler[ToolDescriptor]; a pinner-cli's CLI compiler is a
// Compiler[*cli.Command]. A generic-typed Compile means callers get back a
// concrete []T with no `any` assertion, while still sharing the one "compile a
// catalog" abstraction.
type Compiler[T any] interface {
	// Compile maps every operation in cat to the frontend's native shape as
	// []T. The MCP compiler emits []ToolDescriptor.
