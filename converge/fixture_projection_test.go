...
//
// These tests pin the projection's invariants on fixture operations built
// directly on the root pinner model: stable operation IDs, arg codecs,
// selection groups, AgentConfirm contracts, handler invocability, and no
// Environment filtering in the projection.
...
// degradedHandler returns a handler modeling an operation whose backing
// service is unavailable: it runs and reports a clear error instead of
// panicking.
...
// characterization spot-checks, covering these shapes:
//
...
// stable ID with the same opmesh-owned metadata.
...
func TestProjectFixtureSpotChecks(t *testing.T) {
	...
	// Frontend Environment metadata stays in the pinner model (and mirrored
	// in catalogmeta); the projection is not a filter.
	...
	// registered via RegisterAll (filtering stays the caller's job).
