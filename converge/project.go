// Package converge is the opmesh-convergence projection seam: it projects
...
// same upstream source (the Pinner CLI's internal/catalog). The pinner fork
// carries
// frontend metadata (Environment, MCP Targets/DescFunc, AgentHelp, AgentOnly,
// PositionalOnly, arg Sources) that opmesh deliberately removed; opmesh
// instead treats CLI/MCP presentation as the exclusive concern of the
// boundary adapters that consume it. Instead of maintaining a second fork of
// the operation vocabulary forever, pinner converges onto opmesh: this stage
// introduces opmesh as a dependency and provides a total, panic-free
// projection of this module's Operation content onto the opmesh model so the
...
// boundary adapters (the CLI compiler and the MCP/compiler layers) read it:
//
//   - pinner.Environment (EnvBoth/EnvCLIOnly/EnvLocalOnly/EnvHostedOnly) —
//     surface gating; pinnerops.AssembleCatalogOps already applies it at
//     assembly time and adapters hold the rest.
...
// caller filters before projecting, e.g. pinnerops), does not validate
...
// (AgentHelp, AgentOnly, Sources, PositionalOnly) behind for the boundary
// adapters. Enum and RawSchema values are copied so the projection never
