...
// TestConvergeImportIsolation pins the convergence seam's hard boundary: the
// converge package (non-test sources) may depend only on stdlib plus the two
// in-scope modules — go.lumeweb.com/pinner (its own module) and
// go.lumeweb.com/opmesh. No pterm, urfave, MCP SDK, mcpforge, canimcp, or
// pinner-cli import may sneak in (directly or transitively through another
// local subpackage import in the non-test files).
//
...
// imports (catalogops for the real-provider characterization) do not count
// against the shipped package's isolation.
