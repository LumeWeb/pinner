...
// composed by the same DSL that builds schemas and descriptions. These
// aliases let call-sites and tests keep referencing the pinnermcp names
// without maintaining a parallel definition.
type (
...
func BuildAgentGuide(profile HostProfile, surface pinnerops.Surface, hosted bool) AgentGuide {
...
var flowSurface = map[string]func(pinnerops.Surface) bool{
	"auth":            pinnerops.Surface.AccountOn,
	"vault_create":    pinnerops.Surface.VaultOn,
	"vault_restore":   pinnerops.Surface.VaultOn,
	"vault_upload":    pinnerops.Surface.VaultOn,
	"vault_download":  pinnerops.Surface.VaultOn,
	"vault_share":     pinnerops.Surface.VaultOn,
	"vault_sync":      pinnerops.Surface.VaultOn,
	"upload":          pinnerops.Surface.UploadOn,
	"download":        pinnerops.Surface.UploadOn,
	"pins":            pinnerops.Surface.PinsOn,
	"publish_website": pinnerops.Surface.WebsitesOn,
	"update_website":  pinnerops.Surface.WebsitesOn,
	"ens_publish":     pinnerops.Surface.ENSOn,
}

// filterGuideFlows drops resolved flows whose surface flag is disabled.
func filterGuideFlows(guide AgentGuide, s pinnerops.Surface) AgentGuide {
	if s.IsZero() {
		return guide
	}
	kept := guide.Flows[:0]
...
func AgentGuideDescriptor(surface pinnerops.Surface, hosted bool) model.ToolDescriptor {
