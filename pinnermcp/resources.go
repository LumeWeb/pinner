...
const (
	...
	// WebsiteDNSRequirementsTmplFmt is the fmt.Sprintf form of the
	// dns-requirements template used by prompt handlers that embed a
	// domain-resolved URI at prompt-render time.
	WebsiteDNSRequirementsTmplFmt = "pinner://websites/%s/dns-requirements"
)
...
// and validation-status resources. It is a narrow, locally-declared provider
// interface over the pinner-cli wizard's WebsitesResourceProvider subset of
// cli.WebsitesService: this package must never import pinner-cli, so the
// consumers satisfy the interface from whatever service they already own (the
// method shapes are identical, so an existing implementation satisfies this
...
// resources and resource templates. The wizard session store is
// *session.SessionStore from mcpplane/session; the rest are injected
// interfaces so the resource handlers can call live APIs or mocks.
...
func ResourceDescriptorsForSurface(provs ResourceProviders, surface pinnerops.Surface) ([]model.ResourceDescriptor, []model.ResourceTemplateDescriptor) {
...
func platformDomainsHandler(ws WebsitesResourceProvider) model.ResourceHandler {
	return func(ctx context.Context, req model.ResourceRequest) (model.ResourceResult, error) {
		if ws == nil {
...
// hosting mode, mirroring the CLI's showDNSRecordInstructions output shape.
