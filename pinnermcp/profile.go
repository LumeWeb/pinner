...
// The platform-profile vocabulary of pinnermcp.
//
// pinner-cli kept a Pinner-specific hostenv package that mirrored canimcp's
// feature/host/transport vocabulary one-to-one. This module cannot import
// pinner-cli, and the DSL context needed by the guide/descriptor fragments
// must carry mcpforge's FeatureSet plus the host/transport/hosted facts the
// prose predicates gate on. HostProfile fills that role with canimcp types —
// byte-identical feature strings (kept in lockstep with canimcp.Feature) so
// remote profiles need no translation.

// The pinnermcp feature vocabulary, expressed as mcpforge.Feature. The string
...
// HostProfile is the platform context every pinnermcp description-DSL
...
// the host/transport/hosted constructors hostenv used to own are declared
// here, generic over the DSL context's concrete type.
// ---
...
// composed from that transport's mechanism features (the ProfileForTransport
// fallback of the source hostenv package, sans any host-specific caps).
...
func (e *ProfileAdapterError) Error() string {
	return "pinnermcp: profile of type " + e.ProfileType +
...
// transport-generic stdio profile, matching the source default.
