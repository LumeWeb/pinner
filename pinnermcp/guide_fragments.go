...
// guideFragments hold the prose shared by multiple guide flows/branches so
// playwright copy lives in one place instead of being inlined per branch.
// Each fragment is a complete, self-punctuated mcpforge.DescBuilder so it
...
var publishCidLead = mcpforge.Static[HostProfile]("First obtain a CID via the upload flow's byte-route chooser, then websites_create consumes it directly regardless of which upload tool produced the CID.")
...
// branch.
var htmlRootClause = mcpforge.Static[HostProfile]("Upload with wrap=true and do NOT set an explicit name for HTML — the tool auto-names wrapped HTML to index.html so the site resolves at its root. An explicit name like \"starter-site\" is honored as-is and the site will only be reachable at /starter-site, not /.")

// validateAfterCreateClause is the post-create validation guidance shared by
// the platform-subdomain publish branches.
var validateAfterCreateClause = mcpforge.Static[HostProfile]("After creation, call websites_validate to confirm DNS propagation.")
...
var cdnDeployNoticeClause = mcpforge.Static[HostProfile]("Let the user know that after publishing, the site can take up to 5 minutes to fully deploy to the CDN, so it may not be reachable at its URL immediately even though create/validation succeeded.")
...
var reconcileNoSleep = mcpforge.Static[HostProfile]("On this host there is no sleep tool: if validation is still pending or failing, treat that as reconciliation lag rather than failure and re-call websites_validate after other work, without starting a new flow.")
...
var reconcilePlain = mcpforge.Static[HostProfile]("If validation is still pending or failing, treat it as reconciliation lag and re-call websites_validate after other work, without starting a new flow.")
...
var hnsNamespaceClause = mcpforge.Static[HostProfile]("For a Handshake (alt-root) name such as acme/ (i.e. the user has a Handshake domain, not an ICANN TLD domain), pass {\"namespace\": \"hns\"} alongside the website so the site binds under the HNS namespace.").Then(hnsOnchainClause)
...
var hnsOnchainClause = mcpforge.Static[HostProfile]("Read the created/bound domain's status next:").Sentences(
...
func siteBundleUpload() mcpforge.DescBuilder[HostProfile] {
	return mcpforge.Static[HostProfile]("For a static site bundle (ZIP containing index.html, CSS, JS, images, nested pages): call upload_file").
		Unless(FeatFileHostInput, "with a convert source ({{SOURCES}}) and archive_mode=convert").
		When(FeatFileHostInput, "with the host file argument and archive_mode=convert").
		Sentences(
		...
		SentencesWhenAny([]mcpforge.Feature{FeatFileHostInput, FeatSourcePath},
