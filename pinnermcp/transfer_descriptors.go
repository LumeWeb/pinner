...
// Transfer-tool descriptors: the DESCRIPTOR halves of pinner-cli's
// internal/mcp/core/transfer upload_file.go / upload_data.go /
// download_file.go. The coordinators and executors stay where they already
// live: mcpplane/transfer (source/sink contracts, HTTP upload/download
// coordinators, task manager) and pinnertransfer (the Pinner content-side
// executors). This file injects them via the plain function types
...
//
// pinner-cli's toolforge/tools.go had no mcpforge home: mcpforge is
// consumer-agnostic (feature vocabulary is owned by the consumer), so the
// per-tool description targets live here, expressed over HostProfile. These
// are verbatim ports of toolforge's uploadFileDesc / downloadFileDesc /
// dataURIUploadDesc with toolforge/hostenv swapped for mcpforge/canimcp and
...
func downloadFileDescription(dropWired, tunnelOpenAI bool) string {
	...
	if !ok {
		panic(fmt.Sprintf("pinnermcp: download_file has no matching description target (dropWired=%v tunnelOpenAI=%v)", dropWired, tunnelOpenAI))
	}
	return desc
}
