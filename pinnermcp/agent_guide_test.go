...
// Characterization tests for the agent guide, pinned against
// pinner-cli/internal/mcp/agent_guide_test.go. The guide is host-aware and
// re-resolved per request; the de-globalized analog overlays the surface and
// hosted flags (Config fields) onto the request profile.
...
// TestAgentGuideDescriptorFullSurface mirrors the pinner-cli direct test:
...
func TestAgentGuideDescriptorFullSurface(t *testing.T) {
	desc := AgentGuideDescriptor(pinnerops.FullSurface, false)
...
func TestAgentGuideHostedSurfaceDropsVaultFlows(t *testing.T) {
	guid := BuildAgentGuide(profileForTransport(canimcp.TransportHTTP), pinnerops.HostedSurface, true)
	for _, f := range guid.Flows {
...
func TestAgentGuideModesMatchProfile(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			guide := BuildAgentGuide(tc.profile, pinnerops.FullSurface, false)
...
func TestAgentGuideClaudeWebNoticeScoped(t *testing.T) {
	web := BuildAgentGuide(claudeHTTPProfile(), pinnerops.FullSurface, false)
	...
	desktop := BuildAgentGuide(stdioAppsProfile(), pinnerops.FullSurface, false)
	require.NotContains(t, strings.Join(desktop.Rules, "\n"), "Host capability notice (Claude Web)")

	generic := BuildAgentGuide(profileForTransport(canimcp.TransportHTTP), pinnerops.FullSurface, false)
	require.NotContains(t, strings.Join(generic.Rules, "\n"), "Host capability notice (Claude Web)")

	// Hosted Claude Web is NOT special-cased: no notice, generic mint guidance.
	hostedWeb := BuildAgentGuide(claudeHTTPProfile(), pinnerops.FullSurface, true)
...
func TestAgentGuidePublishChainContainsRealTools(t *testing.T) {
	guid := BuildAgentGuide(profileForTransport(canimcp.TransportHTTP), pinnerops.FullSurface, false)
...
// ---
// Host-profile fixtures mirroring pinner-cli hostenv's static declarations
...
// characterized with the exact feature sets the CLI serves.
// ---
