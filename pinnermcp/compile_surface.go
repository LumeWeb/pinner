...
// operation catalog.
var errNilCatalog = fmt.Errorf("pinnermcp: populateCatalogSurface: nil operation catalog")
...
//
// DE-GLOBALIZED: the source read surface/hosted/profile from package globals
// (surfaceVar / transportFlagsVar / activeHosted); here those are Config
// fields threaded through Assemble, and the projection below is a pure
// function of its arguments. The dispatch/gate plumbing (catalog.Handler,
// CredentialFromContext, the needs_human result mapping) stays where it
// belongs — with the composition root; pinnermcp carries only declared,
