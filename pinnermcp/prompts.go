...
// a failure is a programming error and panics (mirroring the embedded resource
...
func renderPromptTemplate(name string, data sitePromptData) string {
	...
	if err != nil {
		panic("pinnermcp: parse prompt templates: " + err.Error())
	}
	var b bytes.Buffer
	if err := tpl.ExecuteTemplate(&b, name, data); err != nil {
		panic("pinnermcp: render prompt template: " + err.Error())
	}
	return b.String()
}
...
// FullSurface is a convenience alias re-exported from pinnerops so prompt
// callers do not need the pinnerops import for the common full-surface case.
func FullSurface() pinnerops.Surface { return pinnerops.FullSurface }
...
func PromptDescriptorsForSurface(surface pinnerops.Surface) []model.PromptDescriptor {
