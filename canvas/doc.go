...
// views exist and what data and verbs each one speaks). The composition
// roots that consume these contracts — including the standalone `mcp`
// surface and the hosted product — serve the same views through different
// asset adapters; neither may fork the screen set, the payload schemas, or
// the action/state vocabulary.
//
// The package is frontend-neutral and server-neutral. It consumes no Go
// server packages and knows nothing about how any composition root routes
// requests: server packages provide data and actions, frontend packages own
// rendering.
//
...
// no server packages and no composition-root code.
//
