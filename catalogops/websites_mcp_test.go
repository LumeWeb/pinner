...
// profileWith mirrors pinner-cli's hostenv profiles at the pure-feature level:
// pinner never detects hosts, so the characterization tests exercise the DSL
// resolution against an explicit FeatureSet carrying the same feature keys
// ("file-host-input") that hostenv.FeatFileHostInput uses.
...
// profileFileHostFakeHost mimics hostenv.ProfileOpenAITunnel's capability
// half: FeatFileHostInput on. (The legacy transport/relay details stay in
// pinner-cli; only the feature gates affect this description.)
