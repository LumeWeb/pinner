...
//
// These are catalog operations (not hand-written commands): one
// opmesh-model definition feeds every frontend. Each frontend's presentation
// metadata lives on the frontend-metadata boundary packages (Environment
// carve-outs, per-arg agent/CLI metadata), keyed by these operations' stable
// IDs.
package catalogops
