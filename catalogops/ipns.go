...
func IPNSOperations(d IPNSDeps) []pinner.Operation {
...
func ipnsKeysList(d IPNSDeps) pinner.Operation {
	return pinner.NewOperation(pinner.OperationSpec{
		...
		Category:    "ipns", Safety: pinner.SafetyRead, Interaction: pinner.InteractionAgentSafe, Visibility: pinner.VisibilityBoth,
		Positional: "",
		...
		Handler: handler(func(ctx context.Context, input map[string]any) (any, error) {
			...
			if search := pinner.SearchArg(input); search != "" {
...
func ipnsKeysCreate(d IPNSDeps) pinner.Operation {
	return pinner.NewOperation(pinner.OperationSpec{
		...
		Category:    "ipns", Safety: pinner.SafetyMutate, Interaction: pinner.InteractionAgentSafe, Visibility: pinner.VisibilityBoth,
		Positional: "<name>",
		Args: []pinner.OperationArg{
			{Name: "name", Type: pinner.ArgTypeString, Required: true, Help: "Key name"},
			{Name: "key", Type: pinner.ArgTypeString, Sensitive: true, Help: "Private key to import (optional)"},
		},
		Handler: handler(func(ctx context.Context, input map[string]any) (any, error) {
			svc, err := d.service(input)
			if err != nil {
			...
			var key *string
			if k := pinner.StrArg(input, "key", ""); k != "" {
				key = &k
			}
...
func ipnsKeysGet(d IPNSDeps) pinner.Operation {
	return pinner.NewOperation(pinner.OperationSpec{
		...
		Positional: "<id>",
		Args: []pinner.OperationArg{
			{Name: "id", Type: pinner.ArgTypeFlexibleID, Required: true, Help: "Key ID"},
		},
		Handler: handler(func(ctx context.Context, input map[string]any) (any, error) {
			svc, err := d.service(input)
			if err != nil {
			...
			id := pinner.StrFlexibleArg(input, "id", "")
			if id == "" {
...
func ipnsKeysDelete(d IPNSDeps) pinner.Operation {
	return pinner.NewOperation(pinner.OperationSpec{
		...
		Category:    "ipns", Safety: pinner.SafetyDestructive, Interaction: pinner.InteractionAgentSafe, Visibility: pinner.VisibilityBoth,
		Positional: "<id>",
		Args: []pinner.OperationArg{
			{Name: "id", Type: pinner.ArgTypeFlexibleID, Required: true, Help: "Key ID"},
			...
			{Name: "confirm", Type: pinner.ArgTypeBool, AgentRequired: true, Default: "true", Help: "Confirm the destructive delete", AgentHelp: "Must be true to delete the key; this is destructive and cannot be undone. Only a human sets this on confirmation; a model alone cannot confirm a destructive delete."},
		},
		Handler: handler(func(ctx context.Context, input map[string]any) (any, error) {
...
func ipnsPublish(d IPNSDeps) pinner.Operation {
	return pinner.NewOperation(pinner.OperationSpec{
		...
		Positional: "<cid>",
		Args: []pinner.OperationArg{
			{Name: "cid", Type: pinner.ArgTypeString, Required: true, Help: "Content identifier"},
			{Name: "key-name", Type: pinner.ArgTypeString, Help: "IPNS key to publish under (defaults to default key)"},
			{Name: "ttl", Type: pinner.ArgTypeString, Help: "TTL for the published record"},
		},
		Handler: handler(func(ctx context.Context, input map[string]any) (any, error) {
			svc, err := d.service(input)
			if err != nil {
			...
			var ttl *string
			if t := pinner.StrArg(input, "ttl", ""); t != "" {
				ttl = &t
			}
			return svc.Publish(ctx, cid, pinner.StrArg(input, "key-name", ""), ttl)
		}),
	})
}

func ipnsRepublish(d IPNSDeps) pinner.Operation {
	return pinner.NewOperation(pinner.OperationSpec{
		...
		Positional: "",
		Args: []pinner.OperationArg{
			{Name: "key-name", Type: pinner.ArgTypeString, Required: true, Help: "IPNS key to republish"},
		},
		Handler: handler(func(ctx context.Context, input map[string]any) (any, error) {
			svc, err := d.service(input)
			if err != nil {
			...
			return svc.Republish(ctx, pinner.StrArg(input, "key-name", ""))
		}),
	})
}

func ipnsResolve(d IPNSDeps) pinner.Operation {
	return pinner.NewOperation(pinner.OperationSpec{
		...
		Positional: "<name>",
		Args: []pinner.OperationArg{
			{Name: "name", Type: pinner.ArgTypeString, Required: true, Help: "IPNS name to resolve"},
		},
		Handler: handler(func(ctx context.Context, input map[string]any) (any, error) {
			svc, err := d.service(input)
			if err != nil {
