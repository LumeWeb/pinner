...
func vaultVerify(d VaultDeps) opmesh.Operation {
	return opmesh.NewOperation(opmesh.OperationSpec{
		...
		Description: "Check a vault file's integrity: verifies its recorded SHA-256 digest matches and that the object exists on the Sia indexer. Returns an OK/FAIL result with digest and object facts. Does NOT stream or return file content. This is a WRITE, not a pure read: deep=true downloads the full object, recomputes SHA-256, and backfills the digest (persisting it to the object's sealed metadata and the local record) when it is missing; a successful verify also clears any 'lost' status on the file.",
		Category:    CategoryVault,
...
func vaultProfileUse(d VaultDeps) opmesh.Operation {
	return opmesh.NewOperation(opmesh.OperationSpec{
		...
		Description: "Sets the profile used by default when neither an explicit name argument nor the PINNER_PROFILE environment variable selects one. An explicit name argument, or the PINNER_PROFILE environment variable (a host-side setting, not settable via a tool call), still take precedence.",
		Category:    CategoryVault,
		Safety:      opmesh.SafetyMutate,
		Interaction: opmesh.InteractionAgentSafe,
		Visibility:  opmesh.VisibilityBoth,
