...
func authLogin(d AuthDeps) opmesh.Operation {
	return opmesh.NewOperation(opmesh.OperationSpec{
		...
		Description: "Save a provided Pinner.xyz auth token (JWT) as the stored credential and confirm it is valid. This variant has no password or OTP input; interactive or out-of-band sign-in uses the SSO browser flow (auth_sso).",
		Category:    "account",
...
func authLogout(d AuthDeps) opmesh.Operation {
	return opmesh.NewOperation(opmesh.OperationSpec{
		...
		Description: "Remove the stored Pinner.xyz auth token from local config so this installation no longer authenticates. Does not revoke API keys on the server.",
		Category:    "account",
		Safety:      opmesh.SafetyMutate,
		Interaction: opmesh.InteractionAgentSafe,
		Visibility:  opmesh.VisibilityBoth,
