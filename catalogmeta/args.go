// Package catalogmeta defines frontend (CLI/MCP) presentation metadata for
// catalog operations, keyed by the stable operation ID.
//
// It owns everything that describes how a specific frontend presents an
// operation:
//
...
// surface gating without pulling in any frontend machinery.
//
// A consumer (a CLI compiler or MCP compiler, for example)
// looks metadata up by operation ID: absent entries mean "no frontend
// specialization", which is the overwhelmingly common case.
//
// The tables in this package are declarative data; this package does not
// interpret them beyond the lookup helpers below.
package catalogmeta
...
// metadata. It exists so integrity checks and consumers can enumerate the
// keyed table without reaching into package state.
...
// and arg name, or nil when none is declared. Callers also treat a nil result
// as "no frontend specialization" for that argument.
