// Package transfer provides the Pinner/IPFS upload and download
// executors over the transport-neutral coordination layer of
// go.lumeweb.com/mcpplane/transfer.
//
// mcpplane/transfer owns the transport-facing coordination (HTTP
// upload/download coordinators, task manager, source/sink contracts);
// transfer owns the content-side behavior that talks to the Pinner
// services — the stream→upload executor (temp-file buffering, wrap
// sniffing, archive-convert extraction) and the download sink executor
// (root-confined local writes, pre-buffered filedrops). Both halves meet
// on the transfer.UploadHandler and IPFSDownloadHandler contracts.
//
// # Coding conventions
//
//   - This package is a hard boundary: it imports only the standard
//     library, go.lumeweb.com/mcpplane/transfer, go.lumeweb.com/ipfs-content
//     (fs + archive), and this module's core/{uploads,download,config}. It
//     never imports an output-formatting, command-framework, or MCP SDK
//     package.
//   - Executors return plain values/errors; callers own tool descriptors
