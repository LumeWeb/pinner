...
//
// After the opmesh migration, the core operation definitions in catalogops
...
// MCPTargets/DescFunc, Environment flags). MCP presentation metadata that was
// previously inline on those definitions was split out and is keyed here by
...
//   - opTargets — the per-operation MCPTargets data extracted verbatim from
//     the pre-migration catalogops definitions.
...
//
// Everything that describes how the *terminal* presents an operation (urfave
// flags, positional bindings, env sources, human Help variants) stays with
// the CLI boundary in pinner-cli; nothing about it is duplicated here.
//
...
// Target is a per-profile presentation variant for the MCP surface. It is the
// former pinner.Target, relocated to the MCP boundary: Require holds opaque
