...
// Metadata-integrity guards for the opmesh-migration frontend boundaries.
//
// The frontend metadata that used to live inline on the catalogops operation
// definitions now lives keyed by stable operation ID in two boundary packages:
// catalogmeta (env carve-outs, per-arg agent/CLI metadata) and here (MCP
...
// TestArgFrontendSpotChecks pins the semantics-carrying entries that survived
// the migration: the agent-only websites_create platform-claim args, the
...
// TestEnvironmentCarveOuts pins the four surfaced carve-outs exactly as they
// were declared inline pre-migration; every other operation must stay EnvBoth.
