...
// platformProfileDouble mirrors the capability surface of a rich host
// profile: typed
...
// adaptation path (profile.go) rather than a synthetic carrier.
...
// set, mirroring a rich host profile's capability method.
...
// openAIFunnelProfile captures an OpenAI-style host's capability half:
...
// TestAdaptedPlatformProfilePreservesFileHostDescription regresses a
// HIGH-severity finding: a host profile adapted at the package boundary
// must keep its features, so the file-host `file` parameter clause survives.
// The profile double mirrors a rich host profile's capability surface
// (Has/IsTransport/IsHost/CloneFeatures) and does NOT satisfy
// ForgeFeatureCarrier; the test follows the documented adaptation path —
