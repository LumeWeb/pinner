...
// operator-set size cap. It is a deterministic pre-flight check: the stream
// upload executor calls it BEFORE transferring anything, so an oversized tree
// extracted from an archive — whether a single file over the cap or the
// aggregate sum of all files over the cap — is rejected up front rather than
// after partial data has been uploaded.
//
...
//
// (The host-side directory sibling, CheckDirectorySize, which follows symlinks
// via os.Stat, stays in pinner-cli with the local-path upload handlers that are
// its only users.)
