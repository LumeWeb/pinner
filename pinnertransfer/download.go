...
// It is the authenticated IPFS download executor — the same split as
...
// service's Cat. It is the IPFS-only half of pinner-cli's ipfsDownload handler
// with the CLI service-construction factored out — callers (pinner-cli) build
// the concrete download.Service from their config manager; this executor only
// owns the Cat→io.Copy mechanics shared by every surface. The auth gate lives
// in Cat (context-aware), which never rejects an authenticated hosted caller.
