// Package services manages a configured process as an OS service
...
// depending on the library. A few behaviors intentionally differ from other
// service wrappers: uninstall is idempotent when systemd no longer knows the
// unit (keyed on the exit code of a `systemctl list-unit-files` probe), env
// vars from Config.EnvVars and an env file are merged (the env file wins)
// instead of the env file fully replacing them, and literal dollars in
// ExecStart paths/arguments are escaped as $$ (no such escaping is applied
// to Environment= values, where systemd performs no $-expansion).
//
